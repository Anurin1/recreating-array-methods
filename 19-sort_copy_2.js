...
function sort(array, compareFunction) {
	...
	function setAandB (index) {
			if(sort.arguments.length > 1) {
				a = parseInt(arrayToSort[index + 1]);
				b = parseInt(arrayToSort[index]);

			}	
			else {
				a = String(arrayToSort[index + 1]).charCodeAt();
				b = String(arrayToSort[index]).charCodeAt();
			}
	}
	...
	function swapElements(index) {
		...
		if (compareFunctionResult < 0) {
			var helperForB = arrayToSort[index];
			//position b
			arrayToSort[index] = arrayToSort[index + 1];
			//position a
			arrayToSort[index + 1] = helperForB;
		}
	}
